...
namespace WebsiteNoiThat.Areas.Admin.Controllers
{
    public class UserGroupsController : Controller
    {
        ...
        {
            // Kiểm tra nếu GroupId đã tồn tại trong cơ sở dữ liệu
            if (db.UserGroups.Any(ug => ug.GroupId == userGroup.GroupId))
            {
                ModelState.AddModelError("GroupId", "Mã nhóm người dùng đã tồn tại.");
                return View(userGroup);
            }
